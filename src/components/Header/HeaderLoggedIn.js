    ...
    <>{windowSize <= 768 ?
      ...
      : <>
        <div className={`header__navigation-container ${pathname === '/' && 'header__navigation-container_main'}`}>
          <Link className={`link ${pathname === '/movies' && 'header__link_selected'}`} to="/movies">Фильмы</Link>
          <Link className={`link ${pathname === '/saved-movies' && 'header__link_selected'}`} to="/saved-movies">Сохранённые фильмы</Link>
        </div>
